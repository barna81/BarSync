...
runs up to 999:59 (previously 99:59) before wrapping back to 00:00.
The bar count is now hard-capped at 9999 (matching the existing
marker/End Bar ceiling) — reaching bar 10000 wraps the count back to
bar 1, beat 1, resetting the elapsed play time too.

**SCROLL mode: a clearer sense of motion.** With no markers placed,
the SCROLL grid previously gave almost no visual indication that bars
were actually advancing. Blocks of however many bars the current grid
view spans (e.g. at x4: bars 1-4, 5-8, 9-12, ...) now visibly
alternate between plain and a dithered pattern — a continuous stripe
pattern that scrolls left along with the bars. Markers sit inside a
small, clean border, with only the surrounding area dithered. The
"last bar of this block" number now blinks on the beat (instead of
staying on screen permanently) and also appears over a marker and on
the current bar position itself.

**Grid counter and BPM detection now keep running seamlessly while
Operation Setup is open.** Previously, the count froze completely for
as long as the menu was open (on any screen), then jumped abruptly to
the current value once you left.

**New info screens in Operation Setup.** The first time you switch to
SCROLL, or the first time you open SET MARKERS, per menu session, a
brief one-time note appears (SCROLL: capped at a max. of 16 bars; SET
MARKERS: markers only show in grid views x1-x16, or continuously in
SCROLL mode).

**Bug fixes:**

- An aborted Custom+Reset or Custom+Grid button press (e.g. released
before the 1-second threshold was reached) could, depending on
release order, accidentally trigger Reset 1, SET 1.1, or a grid
change. The affected button combos now only trigger once their own
hint sweep was actually visible.
- SET 1.1 used to fall back to a full reset while stopped (clock not
running) — that fallback is now gone entirely; SET 1.1 does nothing
while stopped.
- The reset confirmation showed "RESET 2" permanently while stopped
(and more generally stayed on screen indefinitely instead of
clearing after a moment) — now correctly shows "RESET 1" and clears
itself as expected.
...
any markers you've placed scrolling in from the right. Switchable via `OPERATION SETUP > GRID MODE`.

**New: End Bar.** Defines a bar at which the count should loop back,
stop, or simply carry on - either at a fixed bar number, or
automatically at your last placed marker. Configurable via `OPERATION SETUP > END BAR` / `END BAR ACT`.
...
Affects: `hardware/kicad/BarSync/BarSync.kicad_sch`, `BarSync.kicad_pcb`, `BarSync.net`/`BarSync_Netlist.txt`. Internal
...
Stop+Run on the sequencer got it moving again.** Root cause: `isRunning` is only ever set by an explicit Start/Continue message. If
...
**Bugfix: short button presses were sometimes swallowed entirely.** The old debounce required a new level to stay stable for at least
...
all three buttons equally, since they all go through the same `updateButton()` function.
...
- `Switches > Reset Switch` now has **`RESET 1`** and **`RESET 2`** (both Yes/No, default Yes) — independently configurable per stage
...
The original fix (clearing the UART buffer after waking + calling `MIDI.begin()` again) didn't fix the problem, though - it replaced it
...
real Start command and any clock ticks that had already arrived - `isRunning` stayed false, counters/time/grid stayed completely frozen
...
assignable to the custom button via `Switches > Custom > FUNCTION` (not reachable via the physical reset button). Immediately
...
this and now lives **fixed as its own menu item on page 1** (`SETUP > TIMESIG`), directly switchable between the five existing
...
- Menu is now two-tiered (`SETUP > SWITCHES/DISPLAY/STANDBY/DEFAULTS > detail page`) instead of a flat 6-item list. DIVISOR and the new
...
etc.) renamed to `Custom` (`PIN_BTN_CUSTOM`, `btnCustom`, `onCustomButton`), matching the custom-switch renaming already done in
...
Affects: `hardware/kicad/BarSync/*`, `hardware/BarSync_BOM.md`/`.csv`, `hardware/BarSync_Aufbauanleitung.md`. The firmware (v1.0.1) is unaffected
by these changes.
