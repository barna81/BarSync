...
## Firmware v1.3.2 (22.09.2026)

**Browser-basierter Firmware-Flasher eingeführt.** Firmware kann jetzt
direkt im Browser (Chrome/Edge) geflasht werden, ganz ohne Arduino IDE
und ohne manuelle Treiber-Installation:
https://barna81.github.io/BarSync/flash/. Ein GitHub-Actions-Workflow
baut bei jedem Versions-Tag automatisch eine fertige, geflashte Binary
(kompiliert mit fest gepinnten Toolchain-Versionen: esp32-Core 3.3.12,
MIDI Library 5.0.2, U8g2 2.36.19) und veröffentlicht sie sowohl als
GitHub-Release-Anhang als auch same-origin neben der Flash-Seite
selbst. Die manuelle Route über die Arduino IDE bleibt weiterhin
verfügbar (siehe Kapitel 8 der Aufbauanleitung) und ist jetzt in der
README entsprechend als Alternative aufgeführt.

Diese Version enthält keine funktionalen Änderungen an der Firmware
selbst — `FW_VERSION` wurde ausschließlich angehoben, um den
kompletten Release-Workflow (Tag → automatischer Build → Release →
Flash-Seite) einmal end-to-end mit einer echten Versionsnummer zu
bestätigen.

Betrifft: `.github/workflows/build-firmware.yml`, `docs/flash/*`,
`README.md`/`.de.md`.
...
läuft jetzt bis 999:59 (vorher 99:59), bevor sie auf 00:00 zurückspringt.
Die Taktzahl ist neu hart auf 9999 begrenzt (passend zur bestehenden
Marker-/End-Bar-Obergrenze) — bei Takt 10000 springt die Zählung
automatisch zurück auf Takt 1, Beat 1, inklusive Reset der Spielzeit.

**SCROLL-Modus: deutlichere Bewegungsanzeige.** Da im SCROLL-Grid ohne
Marker bisher kaum zu erkennen war, dass die Takte tatsächlich
weiterlaufen, wechseln sich jetzt Blöcke von jeweils so vielen Takten
wie die aktuelle Grid-Ansicht (z. B. bei x4: Takt 1–4, 5–8, 9–12, ...)
sichtbar zwischen normal und einem gepunkteten Muster ab — ein
fortlaufendes Streifenmuster, das mit den Takten nach links wandert.
Marker sitzen dabei in einem kleinen, sauberen Rahmen, nur der Bereich
drumherum ist gepunktet. Die "letzter Takt dieses Päckchens"-Zahl
blinkt jetzt im Takt (statt dauerhaft angezeigt zu werden) und
erscheint auch über einem Marker sowie auf der aktuellen
Takt-Position selbst.

**Grid-Zähler und BPM-Erkennung laufen jetzt nahtlos weiter, während
Operation Setup geöffnet ist.** Vorher blieb die Zählung komplett
eingefroren, solange das Menü offen war (egal welcher Screen), und
sprang beim Verlassen abrupt auf den aktuellen Stand.

**Neue Hinweis-Screens in Operation Setup.** Beim ersten Wechsel auf
SCROLL bzw. beim ersten Aufruf von SET MARKERS pro Menü-Sitzung
erscheint einmalig ein kurzer Hinweis (SCROLL: auf max. 16 Takte
begrenzt; SET MARKERS: Marker werden nur in den Grid-Ansichten x1–x16
bzw. durchgehend im SCROLL-Modus dargestellt).

**Bugfixes:**

- Ein abgebrochener Custom+Reset- oder Custom+Grid-Tastendruck (z. B.
losgelassen, bevor die 1-Sekunden-Schwelle erreicht war) konnte je
nach Loslass-Reihenfolge versehentlich Reset 1, SET 1.1 oder einen
Grid-Wechsel auslösen. Betroffene Tastenkombinationen lösen jetzt
nur noch aus, wenn ihr eigener Hinweis-Sweep tatsächlich sichtbar war.
- SET 1.1 löste im Stopp-Zustand (Clock nicht laufend) ersatzweise
einen vollen Reset aus — das entfällt jetzt ersatzlos, SET 1.1 tut im
Stopp-Zustand nichts mehr.
- Die Reset-Bestätigung zeigte im Stopp-Zustand dauerhaft "RESET 2"
(bzw. blieb generell dauerhaft stehen, statt nach kurzer Zeit wieder
zu verschwinden) — zeigt jetzt korrekt "RESET 1" und blendet sich wie
gewohnt wieder aus.
...
bereits gesetzter Marker, die von rechts hereinlaufen. Umschaltbar über `OPERATION SETUP > GRID MODE`.
...
**Divisor/Grid-Menü: sichtbare Bezeichnung final auf "Grid" umgestellt.** Nachdem der Taster-Aufdruck und alle Dokumente bereits im letzten
...
Firmware-Bezeichner (`divisorIndex`, `PIN_BTN_DIVISOR`, `SCR_DIVISOR` usw.) bleiben unverändert, wie schon beim Taster-Rename.
...
Betrifft: `hardware/kicad/BarSync/BarSync.kicad_sch`, `BarSync.kicad_pcb`, `BarSync.net`/`BarSync_Netlist.txt`.
Firmware-interne Bezeichner (`divisor`, `divisorIndex`, `PIN_BTN_DIVISOR` etc.) bleiben weiterhin unverändert.
...
nie gestoppt wurde), kamen zwar Clock-Ticks an, aber die allein setzen `isRunning` nicht — BarSync wartete auf eine Nachricht, die nie kommen
...
**Bugfix: kurze Tastendrücke wurden manchmal komplett verschluckt.** Der alte Debounce-Mechanismus verlangte, dass ein neuer Pegel danach
...
`TIMESIG` und `SET 1.1` (**neuer Default**) — `RESET 1`/`RESET 2` entfallen als Custom-Taster-Rolle ersatzlos (der physische
...
- `Switches > Reset Switch` hat jetzt **`RESET 1`** und **`RESET 2`** (beide Ja/Nein, Default Ja) — pro Stufe einzeln einstellbar, ob diese
...
Der ursprüngliche Fix (UART-Puffer nach dem Aufwachen leeren + `MIDI.begin()` erneut aufrufen) hat das Problem aber nicht behoben,
...
frisches, damit unabhängiges Stop+Start-Paar durchkam. `MIDI.begin()` birgt dasselbe Risiko indirekt, da die Bibliothek dabei intern erneut
...
- Menü jetzt zweistufig (`SETUP > SWITCHES/DISPLAY/STANDBY/DEFAULTS > Detailseite`) statt einer flachen 6-Punkte-Liste. DIVISOR und das
...
(keine CV-Hardware auf diesem Board) sowie `DISPLAY > ROTATE` (dieses Gerät bleibt fest im Landscape-Format verbaut, keine
Laufzeit-Drehung nötig).

**Umbenennung (Fortführung von Rev. 1.1):** interne Firmware-Bezeichner
für den Taktart-Taster (`PIN_BTN_TIMESIG`, `btnTimeSig`, `onTimeSigButton` usw.) auf `Custom` umbenannt (`PIN_BTN_CUSTOM`, `btnCustom`, `onCustomButton`), passend zur Custom-Switch-Umbenennung im Schaltplan.
...
Betrifft: `hardware/kicad/BarSync/*`, `hardware/BarSync_BOM.md`/`.csv`, `hardware/BarSync_Aufbauanleitung.md`. Die Firmware (v1.0.1) ist von diesen
